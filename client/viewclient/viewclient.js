...
cream.buildQueryResult = (apps, view) => {
  ...
  apps.forEach((i, j) => {
    ...
    if (cream.build.apps[i]) {
      discount = "";
      if (cream.build.apps[i].price === 0) discount = "Free";
      else if (cream.build.apps[i].price < 0) discount = "No Price";
      else {
...
cream.createRandom = () => {
  ...
  for (let i = 0; i < 100; i++) {
    cream.views.random.push(Object.keys(cream.build.apps)[Math.floor(Math.random() * Object.keys(cream.build.apps).length)]);
  }
}
...
$(document).ready(async() => {
  ...
  $(".filter[data-view]").on("click", e => {
    ...
    if (view === "random") cream.createRandom();
  });

  $(".filter[data-tag]").on("click", e => {
    let papps = [];

    Object.values(cream.build.apps).forEach(i => {
      if (i.tags.indexOf($(e.currentTarget).data("tag")) > -1) papps.push(i);
    });

    papps = papps.map(i => i.appid);

    cream.buildQueryResult(papps, "Tag: " + $(e.currentTarget).data("tag"));
  });
});
